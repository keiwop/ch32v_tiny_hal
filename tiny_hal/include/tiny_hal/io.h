...
#define check_bit(reg, bit)     (reg_ptr(reg) & (1 << bit))

#define sbi(reg, bit)           (reg_ptr(reg) |= (1 << bit))
#define cbi(reg, bit)           (reg_ptr(reg) &= ~(1 << bit))
#define xbi(reg, bit)           (reg_ptr(reg) ^= (1 << bit))

#define sreg(reg, val, bit)     (reg_ptr(reg) |= (val << bit))
#define creg(reg, val, bit)     (reg_ptr(reg) &= ~(val << bit))
#define xreg(reg, val, bit)     (reg_ptr(reg) ^= (val << bit))
...
#define gpio_read_state(pin)    (gpio_ptr(GPIO_INDR, pin) & (1 << (pin % 8)))
...
#define gpio_out_od_10MHz(pin)  gpio_set_cfg(pin, 0b0101)
#define gpio_out_od_2MHz(pin)   gpio_set_cfg(pin, 0b0110)
#define gpio_out_od_50MHz(pin)  gpio_set_cfg(pin, 0b0111)
#define gpio_out_mod_10MHz(pin) gpio_set_cfg(pin, 0b1101)
#define gpio_out_mod_2MHz(pin)  gpio_set_cfg(pin, 0b1110)
#define gpio_out_mod_50MHz(pin) gpio_set_cfg(pin, 0b1111)
...
#define pin_read(pin)           (gpio_read_state(pin) >> (pin % 8))
